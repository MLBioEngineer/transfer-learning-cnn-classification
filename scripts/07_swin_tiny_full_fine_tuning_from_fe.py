"""
Swin Tiny Full Fine-Tuning From Feature Extraction

Auto-extracted from notebook: Full FT Swin -Tiny Feature Extraction.ipynb
...
COMMON_CONFIG = {
    ...
    'lr': 5e-6, # Updated for Full FT
    'optim_fc': 'Adam',
    'batch_size': 16,
    'n_epochs': 50,
    'max_epochs_stop': 30,
    'epochs_patience': 10,
    'weight_decay': 1e-4, # Added for Full FT
    ...
    # individual CNN/Swin model setting for feature extraction
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, config, device, save_path=None):
    ...
    # Suitable for CNN and Swin Full Fine-Tuning training loops
...
def train_single_model_same_config(model_spec, folds_info, base_output_dir, pretrained_model_name=None):
    ...
    print(f"Setup: Full Fine-Tuning for all individual models")
    print(f"==============================")
    ...
    for fold_info in folds_info:
        ...
        optimizer = optim.Adam(
            ...
            lr=fold_config['lr'],
            weight_decay=fold_config.get('weight_decay', 0.0)
        )
...
# %% [cell 23]
model_spec_swin_fe = {
    'model_to_load': 'swin_tiny_patch4_window7_224',
    'model_name': 'Swin_Tiny_Feature_Extraction'
}

# Load all 5 folds into memory
loaded_models = []

for fold in range(1, COMMON_CONFIG['num_folds'] + 1):
    fold_model_path = os.path.join(
        gdrive_base_output_dir,
        model_spec_swin_fe['model_name'],
        f"{model_spec_swin_fe['model_name']}_fold_{fold}.pt"
    )

    # Re-instantiate the base model architecture
    loaded_model = build_timm_model(model_spec_swin_fe['model_to_load'], COMMON_CONFIG)

    # Load the saved state dictionary into the model
    loaded_model.load_state_dict(torch.load(fold_model_path, map_location=device))
    loaded_model = loaded_model.to(device)
    loaded_model.eval() # Set to evaluation mode

    loaded_models.append(loaded_model)
    print(f"Fold {fold} model successfully loaded from {fold_model_path}")

# Save all models into a single dictionary file
all_folds_save_path = '/content/gdrive/MyDrive/my_custom_swin_fe_all_folds.pth'
all_state_dicts = {f'fold_{i+1}': model.state_dict() for i, model in enumerate(loaded_models)}
torch.save(all_state_dicts, all_folds_save_path)
print(f"\nAll 5 fold models saved to a single file: {all_folds_save_path}")

# %% [cell 24]
model_spec_swin_full = {
    'model_to_load': 'swin_tiny_patch4_window7_224',
    'model_name': 'Swin_Tiny_Full_Fine_Tuning_From_FE'
}

COMMON_CONFIG['freeze_backbone'] = False
COMMON_CONFIG['partial_finetune'] = False
COMMON_CONFIG['freeze_batchnorm'] = False
COMMON_CONFIG['lr'] = 5e-6
...
histories_swin_full = train_single_model_same_config(
    model_spec=model_spec_swin_full,
    folds_info=folds_info,
    base_output_dir=gdrive_base_output_dir,
    pretrained_model_name='Swin_Tiny_Feature_Extraction'
)
...
# Load all 5 folds into memory for Full Fine-Tuning
loaded_models_full = []

for fold in range(1, COMMON_CONFIG['num_folds'] + 1):
    fold_model_path = os.path.join(
        gdrive_base_output_dir,
        model_spec_swin_full['model_name'],
        f"{model_spec_swin_full['model_name']}_fold_{fold}.pt"
    )

    # Re-instantiate the base model architecture
    loaded_model = build_timm_model(model_spec_swin_full['model_to_load'], COMMON_CONFIG)
    ...
    loaded_models_full.append(loaded_model)
    print(f"Fold {fold} model successfully loaded from {fold_model_path}")

# Save all models into a single dictionary file
all_folds_save_path_full = '/content/gdrive/MyDrive/my_custom_swin_full_all_folds.pth'
all_state_dicts_full = {f'fold_{i+1}': model.state_dict() for i, model in enumerate(loaded_models_full)}
torch.save(all_state_dicts_full, all_folds_save_path_full)
print(f"\nAll 5 fold models saved to a single file: {all_folds_save_path_full}")

# %% [cell 26]
set_all_seeds(SEED) # Set the random seed to ensure stable, reproducible results
...
print("Individual Fold Performance on Test Set:")
for i, probs in enumerate(all_fold_probs):
...
print("\n===============================================")
print("=== 5-Fold Cross-Validation Test Performance ===")
print("===============================================")
