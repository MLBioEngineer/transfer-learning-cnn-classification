...
Auto-extracted from notebook: Feature Extraction.ipynb
...
# True = Freeze the backbone (Feature Extraction - train only the final layer)
COMMON_CONFIG['freeze_backbone'] = True
COMMON_CONFIG['num_unfrozen_layers_backbone'] = 0

print(f"Starting feature extraction with {model_spec['model_to_load']}...")

# Run the training loop
histories_densenet201 = train_single_model_same_config(
...
print(f"Loading all folds for model: {model_name}...")
...
for fold_to_load in range(1, len(folds_info) + 1):
    ...
    if os.path.exists(model_weights_path):
        # 1. Get the configuration used for this fold
        ...
        print(f"Error: The file {model_weights_path} does not exist. Ensure the model has finished training and saved.")
...
for i, model in enumerate(loaded_models):
    fold_num = i + 1
    fold_config = folds_info[i]['config']
    val_df = folds_info[i]['val_df']

    # Setup transform and dataloader for this fold's validation set
    transform_val = build_val_test_transform(fold_config)
    val_dataset = CustomImageDataset(
        val_df,
        transform=transform_val,
    ...
    results = evaluate_model(
        ...
        device,
        fold_config['class_names'],
        title=f"Fold {fold_num} Validation",
        show_plots=False
    )
...
print(f"\n{'='*30}")
print(f"Average Accuracy across 5 folds: {avg_acc:.4f} (+/- {std_acc:.4f})")
print(f"{'='*30}")
