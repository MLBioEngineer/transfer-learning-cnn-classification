...
Auto-extracted from notebook: convnext feature fraction  .ipynb
...
COMMON_CONFIG = {
    ...
    # individual CNN model setting - Updated for Feature Extraction
...
print("COMMON_CONFIG ready for ConvNeXt Feature Extraction.")
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, config, device, save_path=None):
    ...
    for epoch in range(config['n_epochs']):
        ...
        # --- BATCHNORM HANDLING ---
        # In Feature Extraction (freeze_batchnorm=True), we force BN to eval mode to use ImageNet stats.
        if config.get('freeze_batchnorm'):
            for module in model.modules():
                if isinstance(module, (nn.BatchNorm2d, nn.LayerNorm)):
                    module.eval()
        else:
            # In Full Fine-Tuning, we let all layers (including BN/LN) update stats.
            for module in model.modules():
                if isinstance(module, (nn.BatchNorm2d, nn.LayerNorm)):
                    module.train()
...
def build_timm_model(model_to_load, config):
    ...
    # Handle Freezing Logic for Feature Extraction
    if config.get('freeze_backbone') and not config.get('partial_finetune'):
        ...
        for name, param in model.named_parameters():
            # For ConvNeXt and other timm models, identify the head/classifier
...
def train_single_model_same_config(model_spec, folds_info, base_output_dir, pretrained_model_name=None):
    ...
    print(f"Mode: {'Feature Extraction' if COMMON_CONFIG.get('freeze_backbone') else 'Fine-Tuning'}")
    print(f"==============================")
    ...
    for fold_info in folds_info:
        ...
        # 2. Load Pretrained Weights (if applicable)
        if pretrained_model_name:
            ...
            if os.path.exists(pretrained_path):
                ...
                print(f"Warning: Pretrained weights not found. Using default initialization.")

        # 3. Handle specific finetuning overrides (Partial logic)
        if fold_config.get('partial_finetune'):
            for param in model.parameters(): param.requires_grad = False
            for name, param in model.named_parameters():
                if any(x in name for x in ['features.denseblock4', 'features.norm5', 'classifier', 'head', 'fc']):
                    param.requires_grad = True
    ...
    with open(os.path.join(base_output_dir, f"{model_name}_histories_clean.json Iss"), "w") as f:
...
histories_convnext_fe = train_single_model_same_config(
    model_spec=model_spec_convnext_fe,
...
def evaluate_ensemble(models, dataloader, device, class_names, title="Ensemble Evaluation"):
    for model in models:
        model.eval()
    ...
    prec = precision_score(all_labels, all_ensemble_preds, average='weighted', zero_division=0)
    rec = recall_score(all_labels, all_ensemble_preds, average='weighted', zero_division=0)
    f1 = f1_score(all_labels, all_ensemble_preds, average='weighted', zero_division=0)
    ...
    plt.figure(figsize=(10, 8))
    for i, class_name in enumerate(class_names):
        fpr, tpr, _ = roc_curve(all_labels == i, all_ensemble_probs[:, i])
        roc_auc = auc(fpr, tpr)
        plt.plot(fpr, tpr, label=f"{class_name} AUC={roc_auc:.2f}")
    plt.plot([0, 1], [0, 1], '--', label="Random")
    plt.xlabel("False Positive Rate")
...
# Use the configuration from the first fold for normalization consistency
test_config = folds_info[0]['config']
transform_test = build_val_test_transform(test_config)

test_dataset = CustomImageDataset(clean_test_df, transform=transform_test, labels_map=labels_map)
...
if len(loaded_models) > 0:
    evaluate_ensemble(
        ...
        device=device,
        class_names=class_names,
        title="ConvNeXt Ensemble (Test Set)"
    )
else:
