"""
DeiT Small Feature Extraction

Auto-extracted from notebook: DeiT Small-S Feature Extraction.ipynb
...
COMMON_CONFIG = {
    ...
    'lr': 1e-4,                    # good LR for classifier/head training
    'weight_decay': 1e-4,
    'optim_fc': 'Adam',
    'batch_size': 16,
    'n_epochs': 50,
    ...
    # individual CNN model setting for Feature Extraction
    'freeze_backbone': True,       # freeze pretrained backbone
    'partial_finetune': False,
    'freeze_batchnorm': True,      # important for EfficientNet / BatchNorm models
...
# %% [cell 21]
# title  Training & Evaluation Functions {display-mode: "form"}
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, config, device, save_path=None):
    ...
    for epoch in range(config['n_epochs']):
        ...
        # Force BatchNorm layers into eval mode if freeze_batchnorm is enabled
...
# %% [cell 22]
# title Build Model & Training Setup {display-mode: "form"}
...
def train_single_model_same_config(model_spec, folds_info, base_output_dir, pretrained_model_name=None):
    ...
    for fold_info in folds_info:
        ...
        if pretrained_model_name:
            ...
            print(f"Loading pretrained weights from feature extraction: {pretrained_path}")
...
model_spec_deit_fe = {
    'model_to_load': 'deit_small_patch16_224',
    'model_name': 'DeiT_Small_Feature_Extraction'
}
...
COMMON_CONFIG['freeze_backbone'] = True
COMMON_CONFIG['partial_finetune'] = False
COMMON_CONFIG['freeze_batchnorm'] = True
COMMON_CONFIG['lr'] = 1e-4
...
for fold_info in folds_info:
    ...
    fold_info['config']['lr'] = 1e-4
    fold_info['config']['weight_decay'] = 1e-4

histories_deit_fe = train_single_model_same_config(
    model_spec=model_spec_deit_fe,
...
if os.path.exists(history_save_path):
    ...
    print(f"Loaded {len(histories_loaded)} fold histories for {model_name}.")

    # Plot the loaded histories
    plot_training_curves(histories_loaded, model_name)
else:
    print(f"History file not found at {history_save_path}")
...
model_name = 'DeiT_Small_Feature_Extraction'
model_dir = os.path.join(gdrive_base_output_dir, model_name)

# The models were already saved fold-by-fold during training.
# This will zip the entire directory for easy downloading.
if os.path.exists(model_dir):
    zip_path = f"/content/{model_name}.zip"
    shutil.make_archive(f"/content/{model_name}", 'zip', model_dir)
    print(f"All fold models and configs successfully zipped to: {zip_path}")

    # Uncomment the line below to trigger a download to your local machine
...
# %% [cell 26]
# ============================================================
# Feature Extraction Evaluation Code
# Works for: DeiT / Swin / ConvNeXt / DenseNet / etc.
# ============================================================
...
# ============================================================
# 0. SET RANDOM SEED FOR DETERMINISM
# ============================================================
def set_all_seeds(seed=42):
    random.seed(seed)
...
set_all_seeds(42)
print("Random seed set to 42 for complete determinism.")
...
checkpoint_dir = os.path.join(gdrive_base_output_dir, MODEL_NAME)
...
def load_checkpoint_safely(model, checkpoint_path, device):
    ...
    # remove module. if DataParallel was used
    new_state_dict = {}
...
def create_eval_model(model_to_load, num_classes, checkpoint_path):
    model = timm.create_model(
        model_to_load,
        pretrained=False,
        num_classes=num_classes
    ...
    model = load_checkpoint_safely(model, checkpoint_path, device)
    model = model.to(device)
    model.eval()
...
# ============================================================
# 5. GET VALIDATION LOADER FROM FOLD
# ============================================================

def get_val_loader_from_fold(fold_info):
    if "val_loader" in fold_info:
        return fold_info["val_loader"]

    config = fold_info.get("config", COMMON_CONFIG)

    if "val_df" in fold_info:
        transform_val = build_val_test_transform(config)
        val_dataset = CustomImageDataset(
            fold_info["val_df"],
            transform=transform_val,
    ...
    elif "val_dataset" in fold_info:
        batch_size = config.get("batch_size", 16)
        num_workers = config.get("num_workers", 2)

        return DataLoader(
            fold_info["val_dataset"],
            batch_size=batch_size,
            shuffle=False,
            num_workers=num_workers,
            pin_memory=True
        ...
        raise KeyError(
            "No val_loader, val_dataset, or val_df found inside fold_info. "
            "Please use your existing validation dataloader creation function."
        )


# ============================================================
# 6. 5-FOLD VALIDATION EVALUATION
# ============================================================

fold_results = []

for fold_idx, fold_info in enumerate(folds_info):
    fold_num = fold_idx + 1

    print("\n" + "=" * 70)
    print(f"Evaluating {MODEL_NAME} | Fold {fold_num}")
    print("=" * 70)

    checkpoint_path = os.path.join(
        checkpoint_dir,
        f"{MODEL_NAME}_fold_{fold_num}.pt"
    )

    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"Checkpoint not found: {checkpoint_path}")

    val_loader = get_val_loader_from_fold(fold_info)
    ...
    result = evaluate_model(model, val_loader, class_names)
    ...
    if result["auc_macro"] is not None:
        print(f"Macro AUC         : {result['auc_macro']:.4f}")
...
print("\n" + "=" * 70)
print(f"{MODEL_NAME} 5-Fold Validation Summary")
print("=" * 70)
...
save_path = os.path.join(
    gdrive_base_output_dir,
    f"{MODEL_NAME}_feature_extraction_evaluation_summary.csv"
)
...
print(f"Evaluating 5-fold accuracy for {MODEL_NAME}...")

fold_accuracies = []

for fold_idx, fold_info in enumerate(folds_info):
    fold_num = fold_idx + 1
    checkpoint_path = os.path.join(checkpoint_dir, f"{MODEL_NAME}_fold_{fold_num}.pt")

    if not os.path.exists(checkpoint_path):
        print(f"Checkpoint missing for fold {fold_num}: {checkpoint_path}")
        continue

    # Re-create model and load weights
    model = create_eval_model(MODEL_TO_LOAD, NUM_CLASSES, checkpoint_path)
    model.eval()

    # Prepare dataset and loader with num_workers=0 to prevent multiprocessing crash
    config = fold_info.get("config", COMMON_CONFIG)
    transform_val = build_val_test_transform(config)
    val_dataset = CustomImageDataset(
        fold_info["val_df"],
    ...
    val_loader = DataLoader(
        ...
        batch_size=config.get("batch_size", 16),
        shuffle=False,
        num_workers=0,  # <-- Set to 0 to fix the AssertionError
        pin_memory=True
    )
...
if fold_accuracies:
    ...
    print("=" * 40)
    print(f"Overall 5-Fold Accuracy: {mean_acc:.4f} ± {std_acc:.4f}")
    print("=" * 40)
